...
function rollTeams() {
    ...
    while (roleArrays.length != 0) {
        ...
        if (roleArrays[0].length < 2) {
            console.log(`More players need to have role: ` + roleArrays[0][roleArrays[0].length - 1]);
            return;
        }
        else {
    ...
    players.forEach(player => {
        const li = document.getElementById(`team${player.team}${player.role}`);
        const roleImg = document.createElement("img");
        console.log(player.role)
        roleImg.setAttribute("src", "icons/" + player.role + "On.png");
        // li.appendChild(roleImg);
        // li.innerHTML = player.name;
        li.appendChild(roleImg);
        li.textContent = `<img src="icons/${player.role}On.png"></img>` + player.name;
    })
    return;
};